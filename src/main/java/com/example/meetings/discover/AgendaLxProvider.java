...
/**
 * Lisbon's "Agenda Cultural" via the AgendaLx WordPress REST API. The endpoint
 * is referenced
 * from dados.gov.pt (Município de Lisboa dataset) and exposes ~thousands of
 * curated cultural
 * events — concerts, exhibitions, theatre, guided tours — that you won't find
 * on Ticketmaster.
 *
 * No API key required, but the host blocks requests with default UAs (returns
 * RST), so we send
 * a browser-style User-Agent. Times are only partially structured (string_times
 * like "qua: 21h30");
 * we parse the first hh'h'mm match and fall back to 20:00 Europe/Lisbon when
 * nothing parses.
 */
@Component
public class AgendaLxProvider implements EventProvider {
        ...
        try {
            List<AlxEvent> raw = http.get()
                    ...
                    .onStatus(HttpStatusCode::isError,
                            (req, res) -> log.warn("AgendaLx search failed: {}", res.getStatusCode()))
            ...
            for (AlxEvent e : raw) {
                ...
                if (date == null)
                    continue; // all dates in the past
    ...
    private static java.util.Optional<LocalTime> parseTime(String stringTimes) {
        if (stringTimes == null || stringTimes.isBlank())
